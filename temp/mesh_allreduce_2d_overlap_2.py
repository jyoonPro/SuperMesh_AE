...
class MeshAllreduce2DOverlap2(Allreduce):
    ...
    def get_neighbor(self, node_to_consider, first_dir):
        if first_dir == 'B':
            next_node = self.bottom_nodes[node_to_consider]
        elif first_dir == 'T':
            next_node = self.top_nodes[node_to_consider]
        elif first_dir == 'L':
            next_node = self.left_nodes[node_to_consider]
        else:
            next_node = self.right_nodes[node_to_consider]
        return next_node

    def make_partial_tree(self, node_to_consider, first_dir, second_dir):
        ...
        nodes_to_expand = []
        nodes_to_expand.append(node_to_consider)

        for i in range(per_dim_nodes - 1):
            timestep = time_tracker[node_to_consider]
            next_node = self.get_neighbor(node_to_consider, first_dir)
            tree.append((next_node, node_to_consider, timestep + 1))
            time_tracker[next_node] = timestep + 1
            nodes_to_expand.append(next_node)
            node_to_consider = next_node
        for target_node in nodes_to_expand:
            node_to_consider = target_node
            for i in range(per_dim_nodes - 1):
                timestep = time_tracker[node_to_consider]
                next_node = self.get_neighbor(node_to_consider, second_dir)
                tree.append((next_node, node_to_consider, timestep + 1))
                time_tracker[next_node] = timestep + 1
                node_to_consider = next_node
        return tree
    ...
    def build_trees(self):
        ...
        for node in range(total_nodes):
            ...
            self.left_nodes[node] = left
            self.right_nodes[node] = right
            self.top_nodes[node] = top
            self.bottom_nodes[node] = bottom

        self.template_trees = {}
        self.template_trees[0] = sorted(self.make_partial_tree(0, 'B', 'R'), key=lambda x: x[2])
        self.template_trees[per_dim_nodes - 1] = sorted(self.make_partial_tree(per_dim_nodes - 1, 'L', 'B'), key=lambda x: x[2])
        self.template_trees[self.args.num_hmcs - per_dim_nodes] = sorted(self.make_partial_tree(self.args.num_hmcs - per_dim_nodes, 'R', 'T'), key=lambda x: x[2])
        self.template_trees[self.args.num_hmcs - 1] = sorted(self.make_partial_tree(self.args.num_hmcs - 1, 'T', 'L'), key=lambda x: x[2])
        self.tree_roots = []
        self.tree_roots.append(0)
        self.tree_roots.append(per_dim_nodes - 1)
        self.tree_roots.append(self.args.num_hmcs - per_dim_nodes)
