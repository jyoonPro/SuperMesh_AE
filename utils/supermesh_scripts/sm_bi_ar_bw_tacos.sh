...
for i in ${!syntheticDataSize[@]}; do
  python $SIMHOME/src/simulate.py \
      ...
      --outdir $outdir \
      --booksim-network $topology \
      --booksim-config $SIMHOME/src/booksim2/runfiles/mesh/anynet_${topology}_25_200_v2.cfg \
      --allreduce $allreduce \
      --collective $collective \
      --message-buffer-size 32 \
      --message-size 4096 \
...
for i in ${!syntheticDataSize[@]}; do
  python $SIMHOME/src/simulate.py \
      ...
      --outdir $outdir \
      --booksim-network $topology \
      --booksim-config $SIMHOME/src/booksim2/runfiles/mesh/anynet_${topology}_81_200_v2.cfg \
      --allreduce $allreduce \
      --collective $collective \
      --message-buffer-size 32 \
      --message-size 4096 \
