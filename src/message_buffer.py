...
class Message:
    ...
    '''
    def __init__(self, flow, mid, src, dest, size, msgtype, submsgtype, src_node, src_ni, dest_node, dest_ni, second, priority=0, end=False):
        self.flow = flow
        self.id = mid
        self.src = src
        self.dest = dest
        ...
        self.src_node_id = src_node
        self.computed_src_ni = src_ni
        self.dest_node_id = dest_node
        self.computed_dest_ni = dest_ni
        self.second = second
        self.end = end
...
class MessageBuffer:
    ...
    def enqueue(self, message, cur_cycle, delta):
        ...
        skipped = 0
        while len(self.message_queue[arrival_cycle]) > 0:
            arrival_cycle += 1
            skipped += 1
        if skipped > 0:
            print("Skipped during enqueing " + str(skipped))
