...
//    cout << "Retiring tail flit " << f->id << " " << f->msg->src_node_id << endl;
...
void ScaleTrafficManager::_GeneratePacket(int source, int stype, int vnet, int time, int _flit_size)
{
  int cl = 0;

  Message *message = _input_buffer[source][vnet].front();
  if((message->flow == 56 && message->src == 80 && message->dest==77))
//   || (message->flow == 0 && message->src == 39 && message->dest==20)
//   || (message->flow == 3 && message->src == 34 && message->dest==17)
//   || (message->flow == 1 && message->src == 42 && message->dest==24)
//   || (message->flow == 5 && message->src == 59 && message->dest==40)
//   || (message->flow == 4 && message->src == 55 && message->dest==36))
  {
    _packets_to_watch.insert(_cur_pid);
  }

//  _packets_to_watch.insert(1);
...
//  cout << "Flit size " << _flit_size << endl;
//  cout << "Total flits " << size << endl;
  if (message->subtype == Message::Head || message->subtype == Message::HeadTail)
    size++;
//  cout << "Total flit count" << size << endl;
    ...
    for (int v = 1; v <= _vnets; v++) {
      ...
      if (_input_buffer[input][vnet].empty() == false) {
        ...
        if (_inject_buf_size == 0 ||
          ...
          _GeneratePacket(input, packet_type, vnet, _time, _flit_size);
...
//    cout << "Returning message " << message->src_node_id << endl;
  }
