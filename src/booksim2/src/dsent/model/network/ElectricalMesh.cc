...
namespace DSENT
{
    ...
    void ElectricalMesh::constructModel()
    {
        ...
        // SM_Bi: mesh + 4*(N-1)*2
        number_router_to_router_links += 4 * (10 - 1) * 2;
        // SM_Uni: mesh + 4*(N-1)*1
        //number_router_to_router_links += 4 * (9 - 1) * 1;
        // SM_Alter: mesh + 4(floor(N/2)) * 2
        //number_router_to_router_links += 4 * 5 * 2;
        unsigned int number_router_to_site_links = 2 * number_sites;
        // For SM_Bi
        unsigned int router_number_input_ports = 5 + number_sites_per_router;
        unsigned int router_number_output_ports = 5 + number_sites_per_router;
        // For others
