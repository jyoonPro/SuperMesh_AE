...
//        "/dsent/configs/electrical-mesh.cfg");
    ...
    _config->AddStrField("dsent_emesh_config", string(srcpath) +
        "/dsent/configs/electrical-mesh.cfg");
  } else {
...
int BookSim::IssueMessage(int flow, int src, int dest, int id, int msg_size,
    Message::MessageType type, Message::SubMessageType subtype, int src_node_id, int computed_src_ni, int dest_node_id, int computed_dest_ni, int second, int timestep,
    bool end)
{
  if (id == -1) {
  ...
  Message *message = Message::New(type, subtype, id, flow, src, dest, msg_size, src_node_id, computed_src_ni, dest_node_id, computed_dest_ni, second, timestep, end);
...
tuple<int, int, Message::MessageType, int, int, int, int, int, bool, int> BookSim::PeekMessage(int node, int vnet)
{
  int flow = -1;
  int src = -1;
  ...
  int timestep = -1;
  int src_node_id = -1;
  int computed_src_ni = -1;
  int dest_node_id = -1;
  int computed_dest_ni = -1;
  int second = -1;

  Message *message = _traffic_manager->PeekMessage(node, vnet);
  if (message != nullptr) {
//    cout << "In booksim wrapper " << message->flow << " " << message->src << " " << message->end << " " << message->timestep << " " << message->src_node_id << " " << endl;
    flow = message->flow;
    src = message->src;
    type = message->type;
    end = message->end;
...
//  cout << "In booksim wrapper returning " << flow << " " << src << " " << end << " " << timestep << " " << src_node_id << " " << endl;
  return make_tuple(flow, src, type, src_node_id, computed_src_ni, dest_node_id, computed_dest_ni, second, end, timestep);
}
...
bool BookSim::RunTest()
{
  ...
  for (int src = 0; src < gNodes; src++) {
    ...
    mid = IssueMessage(8, src, message_dests[src], mid, 64, type, subtype, 0, 0, 0, 0, 0, 0);
    assert(mid != -1);
    message_ids[src] = mid;
  ...
  while (!_traffic_manager->Idle() || _outstanding_messages > 0) {
    ...
    for (int node = 0; node < gNodes; node++) {
      for (int vnet = 0; vnet < 2; vnet++) {
        tuple<int, int, Message::MessageType, int, int, int, int, int, bool, int> result = PeekMessage(node, vnet);
...
double BookSim::GetActiveArea()
{
  if (_power_model)
    return _power_model->GetActiveArea();
  else
    return 0.0;
}
