...
public:
  ...
  int IssueMessage(int flow, int src, int dest, int id, int msg_size,
      Message::MessageType type, Message::SubMessageType subtype, int src_node_id, int computed_src_ni,
      int dest_node_id, int computed_dest_ni, int second, int timestep, bool end = true); // msg_size: bytes
  tuple<int, int, Message::MessageType, int, int, int, int, int, bool, int> PeekMessage(int node, int vnet);
