...
void Message::Set(MessageType type_, SubMessageType subtype_, int id_, int
    flow_, int src_, int dest_, int size_, int src_node_id_, int computed_src_ni_, int dest_node_id_, int computed_dest_ni_, int second_, int timestep_, bool end_)
{
  type = type_;
  subtype = subtype_;
  ...
  dest = dest_;
  size = size_;
  src_node_id = src_node_id_;
  computed_src_ni = computed_src_ni_;
  dest_node_id = dest_node_id_;
  computed_dest_ni = computed_dest_ni_;
  second = second_;
  timestep = timestep_;
  end = end_;
}
...
Message * Message::New(MessageType type, SubMessageType subtype, int id, int
    flow, int src, int dest, int size, int src_node_id, int computed_src_ni, int dest_node_id, int computed_dest_ni, int second, int timestep, bool end)
{
  Message * m;
  if (_free.empty()) {
    m = new Message;
  ...
  m->Set(type, subtype, id, flow, src, dest, size, src_node_id, computed_src_ni, dest_node_id, computed_dest_ni, second, timestep, end);
