...
class NPU:
    ...
    def aggregate(self, model_size, group_size):
        ...
        cycles = math.ceil(model_size / pe_list_size)

        return cycles

    def inference(self, model, training=False):
        # only dump when configured, it is too slow
        if self.args.dump:
        ...
        for l in range(model.num_layers):
            ...
            if self.args.dump:
                ...
                sram_write_trace_file = None
                dram_filter_trace_file = None
                dram_ifmap_trace_file = None
    ...
    def backprop(self, model):
        ...
        for l in reversed(range(model.num_layers)):
            ...
            self.back_time[l] = int(total_cycles)
    ...
    def train(self, model):
        ...
        for k in self.back_time.keys():
            self.back_time[k] += inference_cycles
