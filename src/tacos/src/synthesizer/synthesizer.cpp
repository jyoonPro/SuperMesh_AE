...
//    std::string filePath = "chunk_tacos_16_partial_sm_alter.txt";
    std::string filePath = "results/tacos_" + std::to_string(total_nodes) + "_" + topology_name + ".txt";
    std::ofstream outFile(filePath, std::ios::app);
    if (!outFile.is_open()) {
        std::cerr << "Failed to open or create the file." << std::endl;
        return;
    }
    outFile << "[EventTime " << currentTime << " ps] ";
    outFile << "Chunk " << chunk << ": " << src << " -> " << dest << std::endl;
